        ...
        <div class="meetup-lookup centered">
          <div class="level">
            <div class="level-left">
              ...
              <div v-if="searchLocation && meetups && meetups.length > 0" class="level-item">
                <span>Meetups in {{meetups[0].location}}</span>
              </div>
              <div  v-if="category" 
                    @click="cancelCategory"
                    class="level-item">
                <button class="button is-danger">{{category}} X</button>
              </div>
            </div>
    ...
    methods: {
      fetchMeetups() {
        ...
        if(this.searchLocation){
          this.filter['location'] = this.searchLocation.toLowerCase().replace(/[\s,]+/g,'').trim()
        }
