...
export default {
	...
	actions: {
		...
		createMeetup ({rootState}, newMeetup) {
			newMeetup.meetupCreator = rootState.auth.user
			newMeetup.processedLocation = newMeetup.location.toLowerCase().replace(/[\s,]+/g,'').trim()

			return axiosInstance.post('/api/v1/meetups', newMeetup)
					.then(res => res.data)
		},
